...
%       pathbase\CB173\160519\160519_CB173_008\
    ...
    matchstr2 = sprintf('%s_%s_%03i', mouse, date, run);
    matchstr3 = sprintf('%s_%s_%03i', mouse, date(3:end), run);
    
    % Check if the base path exists
    if exist(fullfile(mousedir, matchstr1), 'file') > 0
        out = fullfile(mousedir, matchstr1,'.mat');
        return
    elseif exist(fullfile(mousedir, strcat(matchstr2,'.mat')), 'file') > 0
    ...
    fs = dir(mousedir);
    for i=1:length(fs)
        [~,fname,~] = fileparts(fs(i).name);
        if fs(i).isdir
            if length(fname) > length(matchstr1)
%                 if strcmp(fs(i).name(1:length(matchstr1)), matchstr1)
                if strcmp(fname, matchstr1)
